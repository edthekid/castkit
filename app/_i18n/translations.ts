...
export const translations = {
  ja: {
    ...
    'timer.pomo.howto':     '作業と休憩をくり返して集中と休息のリズムを作るタイマーです。「開始」で作業→休憩の順に自動で切り替わり、設定した回数の作業を終えるごとに長い休憩が入ります。切替時に通知（音・点滅）でお知らせします。作業・休憩の分数は下の設定で変更でき、ブラウザに保存されます。',
    'timer.soundOn':  '通知音オン',
  ...
  en: {
    ...
    'timer.pomo.howto':     'A timer that alternates focus and breaks to build a work rhythm. Press "Start" and it switches focus → break automatically, adding a long break after the set number of focus sessions. It alerts you (sound and flash) on each switch. Adjust the focus/break minutes in the settings below; they are saved in your browser.',
