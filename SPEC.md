...
### 2.7 サイコロ `/dice`
- **基本モード**：サイコロの数（1〜10）だけを選ぶ。面数は6面（d6）固定
- **TRPGモード**：ダイスセット（1d4／2d6／3d6／4d6／1d8／1d10／2d10／1d12／1d20／2d20／1d100）を整ったグリッドのプリセットからワンタップで選ぶ。**1d100（百分率）は本物と同じ2つのd10方式**——十の位（00〜90）＋一の位（0〜9）を振って合計し1〜100を算出（`00`＋`0`＝100）
- **チンチロモード**：3個のサイコロで役を判定（ピンゾロ／ゾロ目／シゴロ／通常役／ヒフミ／目なし）。倍率・強さスコアを算出し、目なしなら1ターン最大3回まで振り直せる（判定エンジンは `app/dice/_chinchiro.ts`）
- **3D物理演算**（Three.js + cannon-es、クライアント動的読み込み）でリアルに転がるアニメーション。画面枠を壁として扱い、ダイスが枠外へ出ないよう反発させる
- 転がり停止後に合計値をポップアップで強調表示
- **標準TRPGダイスは本物の多面体**：d4（四面体）・d8（八面体）・d10／d%（五角二重錐）・d12（十二面体）・d20（二十面体）を実際の多面体形状で描画・物理演算する（形状定義は `app/dice/_diceShapes.ts`）。d6 と非標準の面数はオーソドックスな角丸キューブ（1〜6はドット目・7以上は数字）にフォールバック。個数/面数/モードは **localStorage** に保存
- 出目と合計の履歴を一覧表示。Discord向けフォーマット（例: `🎲 2d6 -> [3, 5] = 8`）でコピー

### 2.8 タイマー `/timer`
- **カウントダウン**：時・分・秒で設定、プリセット（1分／3分／5分／10分／休憩15分）、開始/一時停止/リセット。0到達で通知音（既定オフ・スイッチで切替）＋点滅。配信の待機画面用の**全画面表示（アプリ内オーバーレイ）**に対応
- **ストップウォッチ**：開始/一時停止/リセット・ラップ計測（一覧表示）、1/100秒まで表示
- **指定時刻まで**：目標の日時（datetime-local）までの残り時間をカウントダウン（端末のローカル時刻基準）。「配信開始まであと◯◯」用途。0到達で通知＋全画面表示
- **ポモドーロ**：作業/休憩をループ（既定 25分作業 / 5分休憩・N回ごとに長い休憩）。作業/休憩/長い休憩の分数と間隔をカスタマイズ可、完了サイクル数を表示、フェーズ切替時に通知。設定は **localStorage** に保存
- 経過時間は `Date.now()`（実時刻）基準で算出し、`setInterval` のドリフト・タブ非アクティブ時のスロットリングに強い（復帰時に自動補正）。**タブを切り替えても各モードのタイマーは動き続ける**
...
| `/team-division` `/roulette` `/amida` `/topic` `/debate` `/scoreboard` `/dice` `/timer` | 静的 | 各ツール |
