...
### 2.7 サイコロ `/dice`
- **基本モード**：サイコロの数（1〜10）と面数（2〜100）を数値入力で指定
- **TRPGモード**：よく使うダイスセット（1d20／2d6／3d6／4d6／2d10／1d100）をプリセットで選ぶ、または個数・面数を数値入力。d100（百分率）判定に対応
- **チンチロモード**：3個のサイコロで役を判定（ピンゾロ／ゾロ目／シゴロ／通常役／ヒフミ／目なし）。倍率・強さスコアを算出し、目なしなら1ターン最大3回まで振り直せる（判定エンジンは `app/dice/_chinchiro.ts`）
- **3D物理演算**（Three.js + cannon-es、クライアント動的読み込み）でリアルに転がるアニメーション
- 転がり停止後に合計値をポップアップで強調表示
- オーソドックスな白サイコロ（1〜6はドット目・7以上は数字）。個数/面数/モードは **localStorage** に保存
